...
public class RomajiProcessor {
    ...
    public static String romajiToHiragana(String romaji) {
        ...
        while (start < romaji.length()) {
            int lastFound = -1;
            int lower = 0;
            int upper = ROMAN_INDEXES.length;
            while (upper - lower > 1 && end <= romaji.length()) {
                int lowerKey = calculateRomanIndex(romaji, start, end);
                lower = Arrays.binarySearch(ROMAN_INDEXES, lower, upper, lowerKey);
                if (lower >= 0) {
                    lastFound = lower;
                } else {
                    lower = -lower - 1;
                }
                int upperKey = lowerKey + (1 << (32 - 8 * (end - start)));
                upper = Arrays.binarySearch(ROMAN_INDEXES, lower, upper, upperKey);
                if (upper < 0) {
                    upper = -upper - 1;
                }
                end++;
    ...
    public static RomajiPredictiveResult romajiToHiraganaPredictively(String romaji) {
        Objects.requireNonNull(romaji);
        if (romaji.isEmpty()) {
            return new RomajiPredictiveResult("", Collections.singleton(""));
        }
        StringBuilder hiragana = new StringBuilder();
        int start = 0;
        int end = 1;
        while (start < romaji.length()) {
            ...
            if (end > romaji.length() && upper - lower != 1) {
                ...
                for (int i = lower; i < upper; i++) {
                    ...
                    if (re.remain > 0) {
                        Set<RomanEntry> set2 = findRomanEntryPredicatively(romaji, end - 1 - re.remain);
                        for (RomanEntry re2 : set2) {
                            if (re2.remain == 0) {
        ...
        return new RomajiPredictiveResult(hiragana.toString(), Collections.singleton(""));
    }
    ...
    private static Set<RomanEntry> findRomanEntryPredicatively(String string, int offset) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            endIndex = Arrays.binarySearch(ROMAN_INDEXES, startIndex, endIndex, endKey);
            if (endIndex < 0) {
                endIndex = -endIndex - 1;
            }
            if (endIndex - startIndex == 1) {
                return Collections.singleton(ROMAN_ENTRIES[startIndex]);
            }
        }
        Set<RomanEntry> result = new HashSet<>();
    ...
    private static class RomanEntry {
        ...
        private RomanEntry(String roman, String hiragana, int remain) {
            this.roman = roman;
            this.hiragana = hiragana;
            this.remain = remain;
            this.index = calculateRomanIndex(roman);
        }
    }

    private static int calculateRomanIndex(CharSequence roman) {
        Objects.requireNonNull(roman);
        return calculateRomanIndex(roman, 0, 4);

    }

    private static int calculateRomanIndex(CharSequence roman, int start, int end) {
        int result = 0;
        for (int i = 0; i < 4; i++) {
            int index = i + start;
            int c = index < roman.length() && index < end ? roman.charAt(index) : 0;
            result |= c;
            if (i < 3) {
                result <<= 8;
            }
    ...
    private final static RomanEntry[] ROMAN_ENTRIES = Arrays.stream(new RomanEntry[]{
            new RomanEntry("-", "ー", 0),
            new RomanEntry("~", "〜", 0),
            new RomanEntry(".", "。", 0),
            new RomanEntry(",", "、", 0),
    ...
    }).sorted(Comparator.comparingInt(a -> a.index)).toArray(RomanEntry[]::new);

    private final static int[] ROMAN_INDEXES = Arrays.stream(ROMAN_ENTRIES).mapToInt(e -> e.index).toArray();
}
